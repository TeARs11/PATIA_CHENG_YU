...
@CommandLine.Command(name = "arvandD",
        version = "arvandD 1.0",
...
public class ArvandD extends AbstractPlanner<ADLProblem> {

    /**
     * The class logger.
     */
    private static final Logger LOGGER = LogManager.getLogger(ArvandD.class.getName());
    /**
    ...
    public Plan arvandD(ADLProblem problem) {
    ...
    private Node MonteCarloRandomWalk(final Node node, final ADLProblem problem) {
        ...
        for(int i=0; i<NUM_WALK; i++){
            ...
            if( current.getHeuristic() < hmin){
                ...
                res = (1-alpha) * current.getCost() + alpha * current.getHeuristic();
        ...
        if(smin == null){
            return node;
        }else if(res < weight * current.getCost() + current.getHeuristic()){
            return smin;
        }else{
            return null;
        }
    ...
    public static void main(String[] args) {
        try {
            final Arvand planner = new ArvandD();
